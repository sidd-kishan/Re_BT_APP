/*
 * Decompiled with CFR 0.152.
 * 
 * Could not load the following classes:
 *  kotlin.Metadata
 *  kotlin.io.LinesSequence
 *  kotlin.jvm.internal.Intrinsics
 *  kotlin.jvm.internal.markers.KMappedMarker
 */
package kotlin.io;

import java.util.Iterator;
import java.util.NoSuchElementException;
import kotlin.Metadata;
import kotlin.io.LinesSequence;
import kotlin.jvm.internal.Intrinsics;
import kotlin.jvm.internal.markers.KMappedMarker;

/*
 * Exception performing whole class analysis ignored.
 */
@Metadata(d1={"\u0000\u0017\n\u0000\n\u0002\u0010(\n\u0002\u0010\u000e\n\u0000\n\u0002\u0010\u000b\n\u0002\b\u0004*\u0001\u0000\b\n\u0018\u00002\b\u0012\u0004\u0012\u00020\u00020\u0001J\t\u0010\u0006\u001a\u00020\u0004H\u0096\u0002J\t\u0010\u0007\u001a\u00020\u0002H\u0096\u0002R\u000e\u0010\u0003\u001a\u00020\u0004X\u0082\u000e\u00a2\u0006\u0002\n\u0000R\u0010\u0010\u0005\u001a\u0004\u0018\u00010\u0002X\u0082\u000e\u00a2\u0006\u0002\n\u0000\u00a8\u0006\b"}, d2={"kotlin/io/LinesSequence$iterator$1", "", "", "done", "", "nextValue", "hasNext", "next", "kotlin-stdlib"}, k=1, mv={1, 5, 1})
public static final class LinesSequence.iterator.1
implements Iterator<String>,
KMappedMarker {
    private boolean done;
    private String nextValue;
    final LinesSequence this$0;

    LinesSequence.iterator.1(LinesSequence linesSequence) {
        this.this$0 = linesSequence;
    }

    @Override
    public boolean hasNext() {
        String string = this.nextValue;
        boolean bl = true;
        if (string == null && !this.done) {
            this.nextValue = string = LinesSequence.access$getReader$p((LinesSequence)this.this$0).readLine();
            if (string == null) {
                this.done = true;
            }
        }
        if (this.nextValue != null) return bl;
        bl = false;
        return bl;
    }

    @Override
    public String next() {
        if (!this.hasNext()) throw (Throwable)new NoSuchElementException();
        String string = this.nextValue;
        this.nextValue = null;
        Intrinsics.checkNotNull((Object)string);
        return string;
    }

    @Override
    public void remove() {
        throw new UnsupportedOperationException("Operation is not supported for read-only collection");
    }
}
