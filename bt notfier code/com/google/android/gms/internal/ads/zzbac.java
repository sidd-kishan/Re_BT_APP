/*
 * Decompiled with CFR 0.152.
 * 
 * Could not load the following classes:
 *  com.google.android.gms.internal.ads.zzazg
 *  com.google.android.gms.internal.ads.zzbad
 *  com.google.android.gms.internal.ads.zzgfw
 *  com.google.android.gms.internal.ads.zzgga
 *  com.google.android.gms.internal.ads.zzghj
 */
package com.google.android.gms.internal.ads;

import com.google.android.gms.internal.ads.zzazg;
import com.google.android.gms.internal.ads.zzbad;
import com.google.android.gms.internal.ads.zzgfw;
import com.google.android.gms.internal.ads.zzgga;
import com.google.android.gms.internal.ads.zzghj;

public final class zzbac
extends zzgfw<zzbad, zzbac>
implements zzghj {
    private zzbac() {
        super((zzgga)zzbad.zzc());
    }

    /* synthetic */ zzbac(zzazg zzazg2) {
        super((zzgga)zzbad.zzc());
    }

    public final zzbac zza(String string) {
        if (this.zzb) {
            this.zzae();
            this.zzb = false;
        }
        zzbad.zzd((zzbad)((zzbad)this.zza), (String)string);
        return this;
    }
}
