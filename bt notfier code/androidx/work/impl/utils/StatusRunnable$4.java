...
extends StatusRunnable<List<WorkInfo>> {
    ...
    StatusRunnable.4(WorkManagerImpl workManagerImpl, String string) {
        this.val$workManager = workManagerImpl;
        this.val$name = string;
    }

    List<WorkInfo> runInternal() {
        List list = this.val$workManager.getWorkDatabase().workSpecDao().getWorkStatusPojoForName(this.val$name);
